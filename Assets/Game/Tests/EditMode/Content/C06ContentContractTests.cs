...
namespace Game.Tests.EditMode.Content
{
    ...
    public sealed class C06ContentContractTests
    {
        ...
        public void LegacyLevelStoryFieldsMigrateToCanonicalNames()
        {
            ...
            var level = new LevelDefinition { PreludeStoryId = "pre.c19", PostludeStoryId = "post.c19" };
        ...
        public void GeneratedStoriesAndLevelReferencesUseOneCatalog()
        {
            ...
            try
            {
                ...
                var content = copy.CreateValidatedService(
                    ...
                    {
                        [replacement.StoryId] = replacement,
                        [generatedPost.StoryId] = generatedPost,
                    }
                );
                ...
                var card = new Game.Meta.MetaMapQuery(
                    content,
                    new Game.Progression.ProfileProgressQuery(profile)
        ...
        public void AuthoredCatalogBuildsAndUsesEditedReferences()
        {
            ...
            try
            {
                ...
                Assert.That(
                    content.GetLevel(new Game.Foundation.LevelId(first.LevelId)).PostludeStoryId,
                    Is.EqualTo(first.PreludeStoryId)
                );
                var levelsOfMap = content.GetLevelsForMap(new Game.Foundation.MapId(first.MapId));
                Assert.That(levelsOfMap[levelsOfMap.Count - 1].LevelId, Is.EqualTo(first.LevelId));
            }
            finally
            {
                UnityEngine.Object.DestroyImmediate(copy);
        ...
        public void AuthoredCatalogRejectsInvalidConfiguration(string fault)
        {
            ...
            try
            {
                ...
                {
                    case "duplicate":
                        copy.Levels[1].LevelId = first.LevelId;
                        break;
                    case "unknown-level":
                        first.UnlockRequirement.RequiredLevelIds.Add("official.level.missing");
                        break;
                    case "cycle":
                        first.UnlockRequirement.RequiredLevelIds.Add(copy.Levels[1].LevelId);
                        break;
                    case "unknown-story":
                        first.PostludeStoryId = "official.story.missing";
                        break;
                    case "unknown-map":
                        first.MapId = "official.map.missing";
                        break;
                }
        ...
        public void FirstLevel_PostludeIsValidAndDistinctFromPrelude()
        {
            ...
            Assert.That(
                provider.TryGetStory(new Game.Foundation.StoryId(first.PostludeStoryId), out StoryDefinition story),
                Is.True
            );
            ...
            foreach (var level in provider.Levels)
            {
                ...
                Assert.That(
                    provider.TryGetStory(new Game.Foundation.StoryId(level.PostludeStoryId), out var postlude),
                    Is.True
                );
        ...
        public void OfficialFixtures_ContainSixMapsAndThirtyLevels()
        {
            ...
            Assert.That(
                provider.Levels,
                Has.All.Matches<LevelDefinition>(level => !string.IsNullOrWhiteSpace(level.PreludeStoryId))
            );
            Assert.That(provider.Levels.Select(level => level.PreludeStoryId).Distinct().Count(), Is.EqualTo(30));
        }
        ...
        public void OfficialFixtures_ContainValidBranchingStory()
        {
            ...
            Assert.That(
                StoryDefinitionValidator.TryValidate(
                    provider.TryGetStory(
                        new Game.Foundation.StoryId("official.story.c06_branch"),
                        out Game.Contracts.Content.StoryDefinition story
                    )
                        ? story
                        : null,
                    out string error
