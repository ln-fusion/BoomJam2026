...
关卡资料卡的剧情复播权限按以下事实计算（2026-09-08 补充确认）：

- 关前剧情播放完成或跳过，并成功保存剧情完成事实后，解锁该关的关前剧情复播。
- 关卡通关进度成功保存后，解锁该关的关后剧情复播，并解锁满足 All/Any 前置条件的后续关卡；不要求先看完关后剧情。
- 关后剧情的复播权限与自动播放去重分开判断：复播权限依据关卡完成事实，自动播放是否跳过依据剧情完成事实。
- 未配置对应剧情时不显示该复播入口；尚未解锁时隐藏入口。复播结束或跳过后返回地图，不再次进入 Gameplay，不重复提交通关。
- 模拟失败、未提交成功结果、通关保存失败均不授予新的通关解锁。
- 序章、关前剧情和关后剧情按各自稳定 ID 和用途配置，不以复用的测试对白内容判断剧情类别。
